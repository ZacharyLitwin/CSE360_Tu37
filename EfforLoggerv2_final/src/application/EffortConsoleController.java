...
// Group: TU-37
//Purpose: Controls the effort console screen. allows the insertion of new effort logs into the database
package application;
...
public class EffortConsoleController extends LoginController {
    ...
    // activity button action handlers
	    ...
	    void endActivity(ActionEvent event) {
	    	String sql = "SELECT COUNT(*) AS result_size FROM effort_entries";
    ...
    // other navigation action handlers
	    ...
	    void toPlanningPoker(ActionEvent event) {
	    	try {
	    		defectConsoleBtn.getScene().getWindow().hide();
				Parent root = FXMLLoader.load(getClass().getResource("PLanningPokerStart.fxml"));
				Stage stage = new Stage();
				Scene scene = new Scene(root);
