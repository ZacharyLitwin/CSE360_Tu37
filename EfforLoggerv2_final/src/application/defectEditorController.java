...
// Group: TU-37
//Purpose: Controls the defect screen of effortlogger. It allows the creation, editing, and deletetion of defects from projects
...
public class defectEditorController extends LoginController{
	...
	// Button that goes back to the Effort Console Screen
		@FXML
	...
	void insertNewDefect(Defect newDefect) {
		String sql = "INSERT INTO defects (defectsID, projectName, defectsName, defectsDef, defectsInjected, defectsRemoved, defectCategory, fix, status, empID)" 
						+ "values(?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
		String sql2 = "SELECT COUNT(*) AS result_size FROM defects";
		connect = database.connectDb("empdb");
		try {
			...
			prepare.setString(2, newDefect.projectName);
			prepare.setString(3, newDefect.defectName);
			prepare.setString(4, newDefect.defectDef);
			prepare.setString(5, newDefect.defectsInjected);
			prepare.setString(6, newDefect.defectsRemoved);
			prepare.setString(7, newDefect.defectsCategory);
			prepare.setString(8, newDefect.fix);
			prepare.setString(9, newDefect.status);
			prepare.setInt(10,  currentLoginID);
			prepare.executeUpdate();
		}catch(Exception e) {e.printStackTrace();}
		finally{
    		if (result != null) {
