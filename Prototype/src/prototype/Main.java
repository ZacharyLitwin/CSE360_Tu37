	...
	public void start(Stage LoginScreen) throws Exception {
	    ...
	    loginButton.setOnAction(new EventHandler<ActionEvent>() {
            ...
            public void handle(ActionEvent event) {
		        ...
		        UserInfo current = returnUser(usernameField.getText(), passwordField.getText());
		        boolean found = validUser(usernameField.getText(), passwordField.getText());
		        
                loginAuditEntries.add(new LoginAuditEntry(usernameField.getText(), System.currentTimeMillis(), found));
                
                if (found) { 
                    // Update the table view with the observable list
                	user.add(usernameField.getText());
		...
		newAccountButton.setOnAction(new EventHandler<ActionEvent>() {   	
	        ...
	        public void handle(ActionEvent event) {  
		        ...
		        if (found && current.getRoles() != Authorization.userRoles.MANAGER) {
		        	return;
		        }
	            ...
	            TextField usrRoles = new TextField();
	
	            newAccountGrid.add(newUsernameField, 0, 1);
	            newAccountGrid.add(newPasswordField, 0, 2);
	            newAccountGrid.add(usrRoles, 0, 3);
	
	            newUsernameField.setPromptText("New Username");
	            newPasswordField.setPromptText("New Password");
	            usrRoles.setPromptText("Define User role");
	        	
	        	Button createAccountButton = new Button("Create Account");
	            newAccountGrid.add(createAccountButton, 0, 4);
	            ...
	            createAccountButton.setOnAction(new EventHandler<ActionEvent>() {
	                ...
	                public void handle(ActionEvent event) {
	                    ...
	                    if (newUsername.isEmpty() || newPassword.isEmpty()) {
	                    	...
	                    	if (usrRoles.getText().equals("MANAGER")) {
	                    		existingUsers.add(new UserInfo(newUsername, newPassword, Authorization.userRoles.MANAGER));
	                    	} else if (usrRoles.getText().equals("EMPLOYEE")) {
	                    		existingUsers.add(new UserInfo(newUsername, newPassword, Authorization.userRoles.EMPLOYEE));
	                    	} else if (usrRoles.getText().equals("THIRDPARTY")) {
	                    		existingUsers.add(new UserInfo(newUsername, newPassword, Authorization.userRoles.THIRDPARTY));
	                    	}
